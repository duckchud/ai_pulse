...
- Gold는 성공한 Claude/API 추출을 우선 사용한다. 세션 추출(`codex-session-v1`)은
  해당 story에 성공한 API 추출이 없을 때만 Gold의 fallback으로 사용한다.
...
1. 현재 세션 `(prompt_version, model)` record가 없고 성공한 API 추출도 없는 story를
   제한된 수만큼 JSON으로 출력한다.
...
- 알려진 story에서 raw file을 읽지 못하거나 예상하지 못한 저장 오류가 나면, 사용 가능한
  raw 응답과 오류 메시지를 포함해 `failed` 세션 record를 저장하려고 시도한다. 이 실패
  record조차 저장할 수 없으면 원래 오류 맥락을 포함해 `failed`를 보고한다.
